...
function renderStockStatus(sizes) {
    const outOfStock = sizes.every((size) => {
        return Number(size.stock) === 0;
    })
    if (outOfStock) {
        return `<div class="stockStatus">
                <span style="background-color: red;">Hết hàng</span>
            </div>`
    } else {
        const outOfSize = sizes.find((size) => {
            return Number(size.stock) === 0;
        })
        console.log(outOfSize);
        return outOfSize ? `<div class="stockStatus">
        <span>Hết Size ${outOfSize.size_name}</span>
    </div>`: ' ';
    }
}
    ...
    return products.map((product) =>
        `<div id=${product.product_id} class="product p-2">
            <a href="./product-detail.html?id=${product.product_id}">
                ...
                ${renderStockStatus(product.sizes)}
            </a>
        </div>`).join('');
}
